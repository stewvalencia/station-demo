...
public class StationRestControllerTests {
	...
	public void findAllStations() throws Exception {
		...
		mockMvc.perform(get("/api/stations/")).andExpect(status().isOk()).andExpect(content().contentType(contentType))
				.andExpect(jsonPath("$[0].id", is(stations.get(0).getId().intValue())))
				.andExpect(jsonPath("$[0].stationId", is("TestId"))).andExpect(jsonPath("$[0].name", is("testStation")))
				.andExpect(jsonPath("$[0].hdEnabled", is(true))).andExpect(jsonPath("$[0].callsign", is("KTST")))
	...
	public void findbyStationId() throws Exception {
		...
		mockMvc.perform(get("/api/stations/" + stationId)).andExpect(status().isOk())
				.andExpect(content().contentType(contentType))
				.andExpect(jsonPath("$[0].id", is(stations.get(0).getId().intValue())))
				.andExpect(jsonPath("$[0].stationId", is(stationId)))
				.andExpect(jsonPath("$[0].name", is("News Station"))).andExpect(jsonPath("$[0].hdEnabled", is(false)))
	...
	public void findbyStationName() throws Exception {
		...
		mockMvc.perform(get("/api/stations/names/" + stationName)).andExpect(status().isOk())
				.andExpect(content().contentType(contentType))
				.andExpect(jsonPath("$[0].id", is(stations.get(0).getId().intValue())))
	...
	public void findbyHdEnabled() throws Exception {
		...
		mockMvc.perform(get("/api/stations/hdEnabled/")).andExpect(status().isOk())
				.andExpect(content().contentType(contentType))
				.andExpect(jsonPath("$[0].id", is(stations.get(0).getId().intValue())))
				.andExpect(jsonPath("$[0].stationId", is("TestId"))).andExpect(jsonPath("$[0].name", is("testStation")))
				.andExpect(jsonPath("$[0].hdEnabled", is(true))).andExpect(jsonPath("$[0].callsign", is("KTST")))
	...
	public void addUpdateDeleteStation() throws Exception {
		...
		mockMvc.perform(post("/api/stations/add/" + stationId)).andExpect(status().isOk())
				.andExpect(content().contentType(contentType)).andExpect(jsonPath("$.stationId", is(stationId)));
		List<Station> stations = this.stationRepository.findByStationId(stationId);
		assertFalse(stations.isEmpty());
		mockMvc.perform(
				put("/api/stations/update/" + stationId + "?newStationId=" + stationIdChanged + "&hdEnabled=true"))
				.andExpect(status().isOk()).andExpect(content().contentType(contentType))
				.andExpect(jsonPath("$.stationId", is(stationIdChanged))).andExpect(jsonPath("$.hdEnabled", is(true)));
		...
		assertFalse(stations.isEmpty());
		mockMvc.perform(delete("/api/stations/delete/" + stationIdChanged)).andExpect(status().isOk());
		stations = this.stationRepository.findByStationId(stationId);
		assertTrue(stations.isEmpty());
		stations = this.stationRepository.findByStationId(stationIdChanged);
		assertTrue(stations.isEmpty());
	...
	public void addUpdateDeleteStationWithParameters() throws Exception {
		...
		mockMvc.perform(
				post("/api/stations/add/" + stationId + "?name=" + name + "&hdEnabled=true" + "&callsign=" + callsign))
				.andExpect(status().isOk()).andExpect(content().contentType(contentType))
				.andExpect(jsonPath("$.stationId", is(stationId))).andExpect(jsonPath("$.name", is(name)))
				.andExpect(jsonPath("$.hdEnabled", is(true))).andExpect(jsonPath("$.callsign", is(callsign)));
		List<Station> stations = this.stationRepository.findByStationId(stationId);
		assertFalse(stations.isEmpty());
		mockMvc.perform(put("/api/stations/update/" + stationId + "?newStationId=" + stationIdChanged + "&name="
				+ nameChanged + "&hdEnabled=false" + "&callsign=" + callsignChanged)).andExpect(status().isOk())
				.andExpect(content().contentType(contentType)).andExpect(jsonPath("$.stationId", is(stationIdChanged)))
				.andExpect(jsonPath("$.name", is(nameChanged))).andExpect(jsonPath("$.hdEnabled", is(false)))
	...
	public void UpdateDeleteStationInvalid() throws Exception {
		...
		stations = this.stationRepository.findByStationId(stationIdChanged);
		assertTrue(stations.isEmpty());
		mockMvc.perform(delete("/api/stations/delete/" + stationIdChanged)).andExpect(status().isNotFound());
		stations = this.stationRepository.findByStationId(stationId);
		assertTrue(stations.isEmpty());
		stations = this.stationRepository.findByStationId(stationIdChanged);
		assertTrue(stations.isEmpty());
